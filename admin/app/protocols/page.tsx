    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="max-w-5xl mx-auto w-full px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link
            href="/protocols/new"
            className="flex items-center gap-2 px-6 py-2.5 rounded-full text-white text-sm hover:opacity-90 hover:shadow-lg active:scale-95"
        ...
        <div className="bg-white rounded-2xl overflow-hidden" style={{ border: '1px solid #E8E0F0', boxShadow: '0 2px 16px rgba(79,69,128,0.06)' }}>
          {!protocols || protocols.length === 0 ? (
            ...
            <table className="w-full">
              ...
              <tbody>
                {protocols.map((p, i) => (
                  <tr key={p.id} className="hover:bg-[#F9F7FF] transition-colors duration-150 cursor-pointer" style={{ borderBottom: i < protocols.length - 1 ? '1px solid #E8E0F0' : 'none' }}>
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link href={`/protocols/${p.id}`} style={{ fontFamily: M.family, fontWeight: 600, fontSize: 13, color: '#699BA9' }} className="hover:text-primary hover:underline">Edit →</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
